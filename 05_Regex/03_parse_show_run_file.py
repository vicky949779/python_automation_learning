...
interface_pattern = re.compile(r'interface (\S+)(\n.+?)+\n?\s?ip address ([\d\.]+) ([\d\.]+)') # ip address matching regresion
interface_prop_pattern = re.compile(r'interface (?P<name>\S+)(\n.+?)+\n?\s?ip address (?P<ip_address>[\d\.]+) (?P<mask>[\d\.]+)') #--> ?P<name> this is you will give group name .
...
with open('show_running-config.txt') as file:
    ...
    for interface_conf in interface_prop:
        interface_properties_list.append(interface_conf.groupdict()) #--> .groupdict() this is list all in a dictionary format, this is a proper structure formate. 
    pprint(interface_properties_list, indent=10)  #--> pprint(,indent=10) is give a structre formate of dictionary 
